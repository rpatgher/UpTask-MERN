...
const ModalSearcher = () => {
    ...
    const { modalSearcher, handleSeracher, projects } = useProjects();
    const filteredProjects = search === '' ? [] : projects.filter(project => (
        project.name.toLowerCase().includes(search.toLowerCase())
    ))
    
    return (
        <Transition.Root show={ modalSearcher } as={Fragment} afterLeave={() => {setSearch('')}}>
            <Dialog as="div" className="fixed inset-0 z-10 overflow-y-auto mt-20 p-4 sm:p-20 md:p-20" onClose={handleSeracher}>
                <Transition.Child
                    as={Fragment}
